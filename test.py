...
def main() -> int:
    ...
    if languages != ["python"]:
        print(
            f"Expected hook to use language: python, found: {languages}",
            file=sys.stderr,
        )
        return 1
    ...
    if not hook_rev:
        hook_rev_result = run_capture(
            ["git", "-C", str(repo_root), "rev-parse", "HEAD"], check=False
        )
    ...
    for config_key in ("user.name", "user.email"):
        result = run_capture(
            ["git", "config", "--global", "--get", config_key], check=False
        )
        if result.returncode != 0 or not result.stdout.strip():
            print(
                f"git config --global {config_key} must be set for local test execution",
                file=sys.stderr,
            )
            return 1
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        (sample_repo / ".pre-commit-config.yaml").write_text(
        ...
        if sample_file.read_text(encoding="utf-8") != EXPECTED_OUTPUT_ASCII:
            print(
                "Expected pre-commit to replace U+00D7 with ASCII 'x'", file=sys.stderr
            )
            return 1
